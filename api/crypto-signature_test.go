...
import (
	...
	// crypto package is used to prepare expexted result
...
func TestCompressPubkey(t *testing.T) {
	...
	compressed := CompressPubkey(fromPublicKey(key))
...
func TestPubkeyRandom(t *testing.T) {
	...
	for i := 0; i < runs; i++ {
		...
		pubkey2, err := DecompressPubkey(CompressPubkey(key.Public()))
		if err != nil {
			t.Fatalf("iteration %d: %v", i, err)
		}
		if !reflect.DeepEqual(key.Public(), pubkey2) {
